...
lemma Infinite.tsub_hFinite {x y : ℕ*} (hx : Infinite x) (hy : HFinite y) (_hpos : y < x) :
  ...
  have hf : ∀ᶠ n in nonstandardUltrafilter ℕ, k + m < f n :=
...
/-- Lift a predicate on ℕ to ℕ* via the nonstandardUltrafilter. -/
def liftPred (P : ℕ → Prop) (x : ℕ*) : Prop :=
  Germ.LiftPred P x

/-- Lift a binary relation on ℕ to ℕ* via the nonstandardUltrafilter. -/
...
theorem exists_liftPred_of_not_forall_not {P : ℕ → Prop}
    (h : ¬∀ᶠ n in nonstandardUltrafilter ℕ, ¬P n) : ∃ x : ℕ*, liftPred P x := by
  -- If ¬(∀ᶠ ¬P), then by ultrafilter property, ∀ᶠ P
  have : ∀ᶠ n in nonstandardUltrafilter ℕ, P n := by
...
theorem hyperPrime_ofSeq {f : ℕ → ℕ} :
    HyperPrime (ofSeq f) ↔ ∀ᶠ n in nonstandardUltrafilter ℕ, Nat.Prime (f n) :=
  liftPred_ofSeq
...
theorem liftPred_forall_finset {P : ℕ → ℕ → Prop} {x : ℕ*} {s : Finset ℕ} :
  ...
  · -- Prove by Finset induction that finite ∀ eventually = eventually finite ∀
    intro h
    have aux : ∀ (t : Finset ℕ), (∀ m ∈ t, ∀ᶠ n in nonstandardUltrafilter ℕ, P (f n) m) →
        ∀ᶠ n in nonstandardUltrafilter ℕ, ∀ m ∈ t, P (f n) m := fun t =>
      Finset.induction_on t
        (fun _ => Eventually.of_forall fun _ _ hm => (Finset.notMem_empty _ hm).elim)
        (fun a t' _ha IH h' =>
          have h1 : ∀ᶠ n in nonstandardUltrafilter ℕ, P (f n) a := h' a (Finset.mem_insert_self _ _)
          have h2 : ∀ᶠ n in nonstandardUltrafilter ℕ, ∀ m ∈ t', P (f n) m :=
